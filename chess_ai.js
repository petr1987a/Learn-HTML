...
// Updated with better move ordering and optimized worker pool usage
...
const ChessAI = {
    getSmartMove: function(boardStateFromGame, playerColor, getAllLegalMovesFunc, isKingInCheckFunc) {
        return new Promise((resolve, reject) => {
            ...
            const workerCode = `
                onmessage = function(event) {
                    ...
                    function evaluateBoard(board, getAllLegalMovesFunc, isKingInCheckFunc) {
                        ...
                        // Mate/stalemate
                        const botMoves = getAllLegalMovesFunc('b', board);
                        const humanMoves = getAllLegalMovesFunc('w', board);

                        if (botMoves.length === 0) {
                            if (isKingInCheckFunc('b', board)) return -10000;
                            else return 0;
                        }
                        if (humanMoves.length === 0) {
                            if (isKingInCheckFunc('w', board)) return 10000;
                            else return 0;
                        }
                        return total;
                    }
                    function minimax(board, depth, alpha, beta, maximizingPlayer, getAllLegalMovesFunc, isKingInCheckFunc) {
                        if (depth === 0) {
                        ...
                        for (const move of moves) {
                            ...
                            const result = minimax(newBoard, depth - 1, alpha, beta, !maximizingPlayer, getAllLegalMovesFunc, isKingInCheckFunc);
                            if (maximizingPlayer) {
                    ...
                    const result = minimax(board, depth, -Infinity, Infinity, playerColor === 'w', getAllLegalMovesFunc, isKingInCheckFunc);
            ...
            const workerPool = [];
            const maxWorkers = Math.min(4, navigator.hardwareConcurrency || 2); // Use up to 4 workers or the number of CPU cores
